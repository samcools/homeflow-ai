...
import { Activity, AlertTriangle, BarChart3, Building2, CheckCircle2, ChevronRight, CircleDollarSign, Gauge, Home, MapPinned, Mic, Search, ShieldCheck, Sparkles, Users, Volume2, X } from 'lucide-react';
import './styles.css';
...
type Dashboard = { totalProjects:number; critical:number; atRisk:number; onTrack:number; completed:number; unitsPlanned:number; unitsCompleted:number; budget:number; expenditure:number; overdueMilestones:number; openRisks:number };
...
function App(){
  ...
  useEffect(()=>{ Promise.all([fetch(`${API}/api/projects`).then(r=>r.json()),fetch(`${API}/api/dashboard`).then(r=>r.json())]).then(([p,d])=>{setProjects(p);setDashboard(d);}); },[]);
  ...
  async function ask(text:string){ setBusy(true); setPanel(true); setAnswer('Analysing current project data…');
    try { const r=await fetch(`${API}/api/ai/copilot`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question:text,projectId:selected?.id})}); const d=await r.json(); setAnswer(d.answer || 'No grounded answer available.'); }
    catch { setAnswer('The local API is unavailable. Start the HomeFlow API service and try again.'); } finally { setBusy(false); }
  }
  function startVoice(){
    ...
    r.onresult=(e:any)=>{ const text=e.results[0][0].transcript; setQuery(text); ask(text); };
    r.start();
  }
  ...
  async function loadBrief(){ const r=await fetch(`${API}/api/executive-brief`); setBrief(await r.json()); }

  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark">HF</div><div><b>HomeFlow AI</b><span>by Pyrneo</span></div></div>
      <nav>
        <button className="active"><Gauge/>Command Centre</button><button><Building2/>Projects</button><button><MapPinned/>Delivery Map</button><button><Users/>Contractors</button><button><CircleDollarSign/>Finance</button><button><AlertTriangle/>Risks & Blockers</button><button><Activity/>Recovery Plans</button><button><ShieldCheck/>Audit & Governance</button>
      </nav>
    ...
    <main className="main">
      ...
      {dashboard && <section className="metrics">
        <Metric label="Projects" value={dashboard.totalProjects} icon={Building2}/><Metric label="Critical" value={dashboard.critical} icon={AlertTriangle} tone="red"/><Metric label="At risk" value={dashboard.atRisk} icon={Activity} tone="amber"/><Metric label="Units completed" value={dashboard.unitsCompleted.toLocaleString()} icon={Home} tone="green"/><Metric label="Overdue milestones" value={dashboard.overdueMilestones} icon={CheckCircle2} tone="amber"/><Metric label="Open risks" value={dashboard.openRisks} icon={ShieldCheck} tone="red"/>
      </section>}
      <section className="workspace">
        <div className="portfolio card">
          <div className="card-head"><div><h2>Priority portfolio</h2><p>Project health, budget and physical delivery in one view.</p></div><div className="search"><Search size={17}/><input placeholder="Search project, municipality, contractor…" value={query} onChange={e=>setQuery(e.target.value)}/></div></div>
          <div className="project-list">
            {filtered.map(p=>{ const spend=Math.round(p.expenditure/p.budget*100); return <button className={`project-row ${selected?.id===p.id?'selected':''}`} key={p.id} onClick={()=>setSelected(p)}>
              <div className={`score ${statusClass(p.status)}`}>{p.healthScore}</div><div className="project-main"><b>{p.id} · {p.name}</b><span>{p.municipality} · {p.contractor}</span></div><div className="mini"><span>Spend</span><b>{spend}%</b></div><div className="mini"><span>Physical</span><b>{p.physicalProgress}%</b></div><div className={`status ${statusClass(p.status)}`}>{p.status}</div><ChevronRight size={18}/>
            </button>})}
          </div>
        </div>
        <div className="insight card">
          <div className="card-head"><div><h2>{selected ? selected.id : 'AI Project Health'}</h2><p>{selected ? selected.name : 'Select a project for evidence-grounded insights.'}</p></div><Sparkles size={20}/></div>
          {selected ? <ProjectInsight p={selected} ask={ask}/> : <div className="empty-state"><Gauge size={42}/><b>Explainable project health</b><p>Select any project to see risk drivers, divergence and recommended actions.</p></div>}
        </div>
      </section>
    ...
    {panel && <aside className="copilot"><div className="copilot-head"><div><Sparkles/><div><b>HomeFlow Copilot</b><span>{selected?`Context: ${selected.id}`:'Portfolio context'}</span></div></div><button onClick={()=>setPanel(false)}><X/></button></div><div className="chat"><div className="assistant-bubble">{busy?<span className="typing">•••</span>:answer}</div><div className="grounding"><ShieldCheck size={15}/> Grounded in current demo project records. AI recommendations require human review.</div></div><div className="chat-actions"><button onClick={speak}><Volume2 size={17}/>Speak</button><button onClick={startVoice}><Mic size={17}/>Voice</button></div><form onSubmit={e=>{e.preventDefault();const f=new FormData(e.currentTarget);const t=String(f.get('q')||''); if(t) ask(t); e.currentTarget.reset();}}><input name="q" placeholder="Ask about risk, progress, contractors…"/><button>Ask</button></form></aside>}
  </div>
}

function ProjectInsight({p,ask}:{p:Project;ask:(q:string)=>void}){
  const spend=Math.round(p.expenditure/p.budget*100); const div=spend-p.physicalProgress;
  return <div className="project-insight"><div className="health"><div className={`health-ring ${statusClass(p.status)}`}><strong>{p.healthScore}</strong><span>/100</span></div><div><b>{p.status}</b><span>{Math.round(p.confidence*100)}% confidence · {p.trend}</span></div></div>
    <div className="bars"><label>Budget expenditure <b>{spend}%</b></label><div><i style={{width:`${spend}%`}}></i></div><label>Physical progress <b>{p.physicalProgress}%</b></label><div><i className="physical" style={{width:`${p.physicalProgress}%`}}></i></div></div>
    <div className={`divergence ${div>15?'danger':''}`}><BarChart3 size={18}/><div><b>{div} pt divergence</b><span>{currency(p.expenditure)} spent of {currency(p.budget)}</span></div></div>
    <div className="facts"><div><span>Overdue milestones</span><b>{p.overdueMilestones}</b></div><div><span>Open risks</span><b>{p.openRisks}</b></div><div><span>Evidence age</span><b>{p.evidenceAgeDays}d</b></div><div><span>Units completed</span><b>{p.unitsCompleted}/{p.unitsPlanned}</b></div></div>
    <div className="blocker"><span>Primary blocker</span><b>{p.primaryBlocker}</b></div>
    <div className="insight-actions"><button onClick={()=>ask('Why is this project at risk?')}>Why this score?</button><button onClick={()=>ask('What should we do next?')}>Recommended action</button></div>
  </div>
}
