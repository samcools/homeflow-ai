...
function getOpenAIKey(req:express.Request){const user=currentUser(req);const headerKey=isAdmin(req)?String(req.header('x-openai-api-key')||'').trim():'';return headerKey||String(process.env.OPENAI_API_KEY||'').trim();}
